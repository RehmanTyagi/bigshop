    ...
    return (
        <div className="grid place-items-center m-auto max-md:w-auto w-96 loginPage md:bg-white">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="grid gap-5 border-2 max-md:border-none rounded-md p-7">
                ...
                <p className="text-xs font-medium max-md:text-center">By continuing, you agree to {brandInfo.brandName} <MyLink>Conditions of Use</MyLink> and <MyLink>Privacy Notice.</MyLink></p>
                <p className="text-xs font-medium">Need help? <MyLink>Forgot Password</MyLink></p>
                <div className="grid grid-cols-[1fr,auto,1fr] gap-3">
                    <hr className="mt-2" />
                    <p className="text-xs font-medium text-center">New to {brandInfo.brandName}? </p>
                    <hr className="mt-2" />
                </div>
                <MyLink className="text-xs max-md:text-white md:text-black hover:bg-gray-50 hover:no-underline border-[1px] border-gray-300 p-1.5 rounded text-center shadow-inner">Create your {brandInfo.brandName} account!</MyLink>
            </form>
        </div>
    )
}
